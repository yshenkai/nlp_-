...
def save_file(htmlbody,label,mode="data"):
    with open("data_xunlian/chandi/train_{}_line_wise.data".format(mode), 'a+', encoding="utf-8") as f:
                ...
                for T in Ts:
                    ...
                    for item in items:
                        ...
                        for name in names:
                            ...
                            if str_name in labels:
                                setitemname.add(str_name)
                    if len(setitemname)>=7:
                        for item in items:
                            names=item.getElementsByTagName("name")
                            for name in names:
                                str_name=name.childNodes[0].data
...
if __name__=="__main__":
    constact_qiye_data("C:\\Users\\Administrator.USER-20180601PP\\Desktop\\整合\\整合")